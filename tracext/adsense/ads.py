    ...
    def __init__(self):
        try:
            ...
            if self.env.is_component_enabled(adspanel.web_ui.AdsPanel):
                raise TracError("You need to disable the AdsPanel plugin in "
                                "order to use the Adsense4Trac plugin")
        except ImportError:
            pass
...
jQuery(document).ready(function() {
    ...
    jQuery('a.toggle_ads').bind('click', function() {
        ...
        ads_div_id = self.config.get('adsense', 'ads_div_id', 'main')
        if ads_div_id == 'main':
    ...
    def dont_show_ads(self, req):
        if req.session.get('adspanel.state') == 'hidden':
            ...
            if self.config.getbool('adsense', 'hide_for_authenticated', False):
                return True
        return False
